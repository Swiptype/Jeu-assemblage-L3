...
/**
 * Classe représentant une pièce de puzzle de type "L".
 * Cette pièce est en forme de L et peut être orientée à 0°, 90°, 180° et 270°.
 */
public class LPiece extends AbstractPiecePuzzle {

    /**
     * Constructeur de la classe LPiece.
     *
     * @param hauteur La hauteur de la pièce de type "L".
     * @param largeur La largeur de la pièce de type "L".
     */
    public LPiece(int hauteur, int largeur) {
    ...
    protected void createAnglesSetOfCoords() {
        ...
        for (int i = moitieHauteur * -1; i < (moitieHauteur + resteHauteur); i++) {
            for (int j = moitieLargeur * -1; j < (moitieLargeur + resteLargeur); j++) {
                ...
                if (j == (moitieLargeur + resteLargeur - 1)) {
                    this.anglesSetOfCoords.get(180).add(new Coordonnee(i, j));
                    this.anglesSetOfCoords.get(270).add(new Coordonnee(j, i));
                }
            }
